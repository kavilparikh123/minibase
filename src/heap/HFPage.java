...
public class HFPage extends Page implements ConstSlot, GlobalConst {
	...
	public Map getMap(MID mid) throws IOException, InvalidSlotNumberException {
		...
		byte[] recBytes;
		PageId pageNo = new PageId();
		pageNo.pid = mid.getPageNo().pid;
		int curPageId = Convert.getIntValue(CUR_PAGE, data);
		int slotNo = mid.getSlotNo();
		recLen = getSlotLength(slotNo);
		short slotCount = Convert.getShortValue(SLOT_CNT, data);
		if ((slotNo >= 0) && (slotNo < slotCount) && (recLen > 0) && (pageNo.pid == curPageId)) {
			offset = getSlotOffset(slotNo);
			recBytes = new byte[recLen];
			System.arraycopy(data, offset, recBytes, 0, recLen);
			Map map = new Map(recBytes, 0, recLen);
			return map;
		} else {
			throw new InvalidSlotNumberException(null, "HEAPFILE: INVALID_SLOTNO");
		}
	...
	public Map returnMap(MID mid) throws IOException, InvalidSlotNumberException {
		short recLen;
		short offset;
		PageId pageNo = new PageId();
		pageNo.pid = mid.getPageNo().pid;
	...
	public MID insertMap(byte[] record) throws IOException {
		MID mid = new MID();
		int recordLength = record.length;
		int requiredSpace = recordLength + SIZE_OF_SLOT;
		freeSpace = Convert.getShortValue(FREE_SPACE, data);
		if (requiredSpace > freeSpace) {
			...
			usedPtr = Convert.getShortValue(USED_PTR, data);
			usedPtr -= recordLength;
			Convert.setShortValue(usedPtr, USED_PTR, data);
			setSlot(i, recordLength, usedPtr);
			System.arraycopy(record, 0, data, usedPtr, recordLength);
			curPage.pid = Convert.getIntValue(CUR_PAGE, data);
	...
	public void deleteMap(MID mid) throws IOException, InvalidSlotNumberException {
		int slotNumber = mid.getSlotNo();
		short recordLength = getSlotLength(slotNumber);
		short slotCount = Convert.getShortValue(SLOT_CNT, data);
		if ((slotNumber >= 0) && (slotNumber < slotCount) && (recordLength > 0)) {
			int offset = getSlotOffset(slotNumber);
			short usedPointer = Convert.getShortValue(USED_PTR, data);
			int newSpot = usedPointer + recordLength;
			int size = offset - usedPointer;
			System.arraycopy(data, usedPointer, data, newSpot, size);
			int i, n, checkOffset;
			for (i = 0, n = DPFIXED; i < slotCount; n += SIZE_OF_SLOT, i++) {
				if ((getSlotLength(i) >= 0)) {
					checkOffset = getSlotOffset(i);
					if (checkOffset < offset) {
						checkOffset += recordLength;
						Convert.setShortValue((short) checkOffset, n + 2, data);
					}
				}
			}
			usedPointer += recordLength;
			Convert.setShortValue(usedPointer, USED_PTR, data);
			short freeSpace = Convert.getShortValue(FREE_SPACE, data);
			freeSpace += recordLength;
			Convert.setShortValue(freeSpace, FREE_SPACE, data);
			setSlot(slotNumber, EMPTY_SLOT, 0);
		} else {
			throw new InvalidSlotNumberException(null, "HEAPFILE: INVALID_SLOTNO");
		}
	}

	public MID nextMap(MID currentRid) throws IOException {
		MID nextMID = new MID();
		short slotCount = Convert.getShortValue(SLOT_CNT, data);
		int i = currentRid.getSlotNo();
		short slotLength;
		for (i++; i < slotCount; i++) {
			slotLength = getSlotLength(i);
			if (slotLength != EMPTY_SLOT)
				break;
		}
		if (i >= slotCount)
			return null;
