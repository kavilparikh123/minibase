...
public class Heapfile implements Filetype, GlobalConst {
	 ...
	 * false.
	 */
	private boolean _findDataPage(RID rid, PageId dirPageId, HFPage dirpage, PageId dataPageId, HFPage datapage,
	 ...
	 */
	public Tuple getRecord(RID rid) throws InvalidSlotNumberException, InvalidTupleSizeException, HFException,
	...
	public Map getMap(MID mid) throws Exception {
		boolean mapFound;
		HFPage dirPage = new HFPage();
		...
		mapFound = _findDataPageMap(mid, dirPageId, dirPage, dataPageId, dataPage, dataPageRid);

		if (!mapFound)
			return null;

		Map map = new Map();
		map = dataPage.getMap(mid);
	...
	private boolean _findDataPageMap(MID mid, PageId dirPageId, HFPage dirPage, PageId dataPageId, HFPage dataPage,
			RID dataPageRid)
			throws InvalidTupleSizeException, InvalidSlotNumberException, IOException, HFBufMgrException, HFException {
		...
		pinPage(currentPageId, currentPage, false);

		Map map = new Map();
		Tuple tuple = new Tuple();

		while (currentPageId.pid != INVALID_PAGE) {
			for (currentDataPageRid = currentPage
					.firstRecord(); currentDataPageRid != null; currentDataPageRid = currentPage
							.nextRecord(currentDataPageRid)) {
				try {
					tuple = currentPage.getRecord(currentDataPageRid);
				} catch (InvalidSlotNumberException e) {
					return false;
				}

				DataPageInfo dataPageInfo = new DataPageInfo(tuple);
				try {
					pinPage(dataPageInfo.pageId, currentDataPage, false);
				} catch (Exception e) {
				...
				if (dataPageInfo.pageId.pid == mid.getPageNo().pid) {
					map = currentDataPage.returnMap(mid);

					dirPage.setpage(currentPage.getpage());
					dirPageId.pid = currentPageId.pid;

					dataPage.setpage(currentDataPage.getpage());
					dataPageId.pid = dataPageInfo.pageId.pid;
			...
			try {
				...
				throw new HFException(e, "heapfile, findDataPage, unpinpage failed");
			}
			...
			if (currentPageId.pid != INVALID_PAGE) {
				pinPage(currentPageId, currentPage, false);
				if (currentPage == null)
					throw new HFException(null, "pinPage returned null page");
			}
	...
	public MID insertMap(byte[] mp)
			throws InvalidSlotNumberException, InvalidTupleSizeException, InvalidMapSizeException,
			SpaceNotAvailableException, HFException, HFBufMgrException, HFDiskMgrException, IOException {
		int l = mp.length;
		boolean f;
		RID cR = new RID();
		Page p = new Page();
		HFPage cDP = new HFPage();
		HFPage cDirP = new HFPage();
		HFPage nDirP = new HFPage();
		PageId cDirId = new PageId(_firstDirPageId.pid);
		PageId nDirId = new PageId();
		pinPage(cDirId, cDirP, false);
		f = false;
		Tuple t;
		DataPageInfo dpi = new DataPageInfo();
		while (f == false) {
			for (cR = cDirP.firstRecord(); cR != null; cR = cDirP.nextRecord(cR)) {
				t = cDirP.getRecord(cR);
				dpi = new DataPageInfo(t);
				if (l <= dpi.availspace) {
					f = true;
					break;
				}
			...
			if (f == false) {
				if (cDirP.available_space() >= DataPageInfo.size) {
					cDP = _newDatapage(dpi);
					t = dpi.convertToTuple();
					byte[] td = t.getTupleByteArray();
					cR = cDirP.insertRecord(td);
					if (cR == null)
						throw new HFException(null, "no space to insert rec.");
					f = true;
				} else {
					nDirId = cDirP.getNextPage();
					if (nDirId.pid != INVALID_PAGE) {
						unpinPage(cDirId, false);
						cDirId.pid = nDirId.pid;
						pinPage(cDirId, cDirP, false);
					} else {
						nDirId = newPage(p, 1);
						if (nDirId == null)
							throw new HFException(null, "can't new pae");
						nDirP.init(nDirId, p);
						PageId tp = new PageId(INVALID_PAGE);
						nDirP.setNextPage(tp);
						nDirP.setPrevPage(cDirId);
						cDirP.setNextPage(nDirId);
						unpinPage(cDirId, true);
						cDirId.pid = nDirId.pid;
						cDirP = new HFPage(nDirP);
					}
				}
			} else {
				pinPage(dpi.pageId, cDP, false);
			}
		}
		if ((dpi.pageId).pid == INVALID_PAGE)
			throw new HFException(null, "invalid PageId");
		if (!(cDP.available_space() >= l))
			throw new SpaceNotAvailableException(null, "no available space");
		if (cDP == null)
			throw new HFException(null, "can't find Data page");
		MID m;
		m = cDP.insertMap(mp);
		dpi.recct++;
		dpi.availspace = cDP.available_space();
		unpinPage(dpi.pageId, true);
		t = cDirP.returnRecord(cR);
		DataPageInfo dpi_odp = new DataPageInfo(t);
		dpi_odp.availspace = dpi.availspace;
		dpi_odp.recct = dpi.recct;
		dpi_odp.pageId.pid = dpi.pageId.pid;
		dpi_odp.flushToTuple();
		unpinPage(cDirId, true);
		return m;
	}

	public boolean deleteMap(MID mid) throws Exception {
		boolean status;
		HFPage p1 = new HFPage();
		PageId id1 = new PageId();
		HFPage p2 = new HFPage();
		PageId id2 = new PageId();
		RID rid = new RID();
		status = _findDataPageMap(mid, id1, p1, id2, p2, rid);
		if (status != true)
			return status;
		Tuple t;
		t = p1.returnRecord(rid);
		DataPageInfo info = new DataPageInfo(t);
		p2.deleteMap(mid);
		info.recct--;
		info.flushToTuple();
		if (info.recct >= 1) {
			info.availspace = p2.available_space();
			info.flushToTuple();
			unpinPage(id2, true);
			unpinPage(id1, true);
		} else {
			...
			rid = p1.firstRecord();
			PageId pageId;
			pageId = p1.getPrevPage();
			if ((rid == null) && (pageId.pid != INVALID_PAGE)) {
				HFPage prev = new HFPage();
				pinPage(pageId, prev, false);
				pageId = p1.getNextPage();
				prev.setNextPage(pageId);
				pageId = p1.getPrevPage();
				...
				if (pageId.pid != INVALID_PAGE) {
					...
					pageId = p1.getNextPage();
					pinPage(pageId, next, false);
					pageId = p1.getPrevPage();
					next.setPrevPage(pageId);
					pageId = p1.getNextPage();
					unpinPage(pageId, true);
				}
				unpinPage(id1, false);
	...
	public MapScan openMapScan() throws InvalidTupleSizeException, IOException {
