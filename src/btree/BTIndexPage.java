...
public class BTIndexPage extends BTSortedPage {
	...
	public KeyDataEntry getFirst(RID rid) throws IteratorException {
		...
		try {
			...
			entry = BT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType, NodeType.INDEX);
	...
	int getSibling(KeyClass key, PageId pageNo) throws IndexFullDeleteException {
		...
		try {
			...
			for (i = getSlotCnt() - 1; i >= 0; i--) {
				...
				if (BT.keyCompare(key, entry.key) >= 0) {
					if (i != 0) {
						entry = BT.getEntryFromBytes(getpage(), getSlotOffset(i - 1), getSlotLength(i - 1), keyType,
								NodeType.INDEX);
	 ...
	 */
	boolean redistribute(BTIndexPage indexPage, BTIndexPage parentIndexPage, int direction, KeyClass deletedKey)
