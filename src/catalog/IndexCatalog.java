...
public class IndexCatalog extends Heapfile implements GlobalConst, Catalogglobal {
	...
	// OPEN INDEX CATALOG
	IndexCatalog(String filename) throws IOException, BufMgrException, DiskMgrException, Exception {
		super(filename);
	...
	public void addIndex(String relation, String attrName, IndexType accessType, int buckets)
			throws IOException, Catalogioerror, Cataloghferror, Catalogmissparam, Catalogattrnotfound, Catalogbadtype,
			Catalognomem, Catalogindexnotfound, IndexCatalogException, java.lang.Exception {
		RID rid = null;
		...
		if ((attrRec.attrType.attrType != AttrType.attrInteger) && (attrRec.attrType.attrType != AttrType.attrString))
	...
	public String buildIndexName(String relation, String attrName, IndexType accessType) {
		...
		sizeName = relation.length() + accessName.length() + attrName.length() + (3 * sizeOfByte);
	...
	public int getAttrIndexes(String relation, String attrName, int indexCnt, IndexDesc[] indexes)
			throws Catalogmissparam, Catalogioerror, Cataloghferror, IOException, Catalognomem, Catalogindexnotfound,
			Catalogattrnotfound, IndexCatalogException {
	...
	public void getInfo(String relation, String attrName, IndexType accessType, IndexDesc record)
			throws Catalogmissparam, Catalogioerror, Cataloghferror, IOException, Catalogattrnotfound, Exception {
		int recSize;
		RID rid = null;
		Scan pscan = null;
		...
		while (true) {
			...
			if (record.relName.equalsIgnoreCase(relation) == true && record.attrName.equalsIgnoreCase(attrName) == true
	...
	public int getRelInfo(String relation, int indexCnt, IndexDesc[] indexes)
			throws Catalogmissparam, Catalogioerror, Cataloghferror, Catalogindexnotfound, IOException, Catalognomem,
			Catalogattrnotfound, IndexCatalogException, RelCatalogException, Catalogrelnotfound {
		RelDesc record = null;
		int status;
		int recSize;
		RID rid = null;
