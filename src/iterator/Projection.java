...
public class Projection {
	 ...
	 */
	public static void Join(Tuple t1, AttrType type1[], Tuple t2, AttrType type2[], Tuple Jtuple, FldSpec perm_mat[],
	...
	public static void Project(Tuple t1, AttrType type1[], Tuple Jtuple, FldSpec perm_mat[], int nOutFlds)
	...
	public static void project(Map map, AttrType[] attrTypes, Map jMap, FldSpec[] permMat)
			throws UnknowAttrType, WrongPermat, FieldNumberOutOfBoundException, IOException {
		for (int i = 0; i < 4; i++) {
			switch (permMat[i].relation.key) {
			case RelSpec.outer: 
				switch (attrTypes[permMat[i].offset - 1].attrType) {
				case AttrType.attrInteger:
					jMap.setTimeStamp(map.getTimeStamp());
					break;
				case AttrType.attrString:
					jMap.setStrFld(i + 1, map.getStringField((short) permMat[i].offset));
					break;
				default:
					throw new UnknowAttrType("Don't know how to handle attrSymbol, attrNull");
				}
				break;
			default:
				throw new WrongPermat("Something is wrong in permMat");
			}
		}
		return;
	}
