...
public abstract class Iterator implements Flags {
	...
	/**
	 * a flag to indicate whether this iterator has been closed. it is set to true
	 * the first time the <code>close()</code> function is called. multiple calls to
	 * the <code>close()</code> function will not be a problem.
	 */
	...
	/**
	 * free all the buffer pages we requested earlier. should be called in the
	 * destructor
	 * 
	 ...
	 */
	public abstract Tuple get_next() throws IOException, JoinsException, IndexException, InvalidTupleSizeException,
